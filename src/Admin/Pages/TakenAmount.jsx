...
const TakenAmount = () => {
  ...
  const fetchAmounts = async (farmerId) => {
    setFetchLoading(true);
    try {
      const response = await api.get(
        `/farm/?action=getFarmerAmount&farmer=${farmerId}`
      );
      const amountsData = response.data.data || [];
      const totalAmountData = amountsData.reduce(
        (sum, item) => sum + (parseFloat(item.taken_amount) || 0),
  ...
  const handleSearch = (e) => {
    const query = e.target.value.toLowerCase();
    setSearchQuery(query);
    const filtered = amountHistory.filter(
      (history) =>
        formatDateForDisplay(history.date).toLowerCase().includes(query) ||
        history.amount.toString().toLowerCase().includes(query)
    );
    setFilteredAmountHistory(filtered);
    setCurrentPage(1);
  };
  ...
  const handleNext = () => {
    const totalPages = Math.ceil(filteredAmountHistory.length / pageSize);
  ...
  const totalPages = Math.ceil(filteredAmountHistory.length / pageSize);
  const currentHistory = filteredAmountHistory.slice(
