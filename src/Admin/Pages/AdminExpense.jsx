...
//     description: "",
//     farmer_id: defaultFarmerId,
//     date: "", // Added date field to formData
//     date_created: "",
//     manager_id: null,
//   });
//   const [isEditing, setIsEditing] = useState(true);
...
//         id: Number(item.id),
//         manager_id: item.manager_id || selectedManagerId,
//         date: item.date || "N/A", // Ensure date is set
//       });
//     }
//     setIsEditing(false);
//     setIsOpen(true);
...
//         id: Number(item.id),
//         manager_id: item.manager_id || selectedManagerId,
//         date: item.date || "", // Ensure date is set for editing
//       });
//     }
//     setIsEditing(true);
//     setIsOpen(true);
...
function ExpenseForm() {
  ...
  const fetchAdminExpenses = async ( page = 1) => {
    setLoading(true);
    try {
      const user = JSON.parse(localStorage.getItem("user"));
      const farmerId = user?.farmer_id;
      ...
      let farmerAmountId = null;
      try {
        const amountUrl = `/farm/?action=getFarmerAmount&farmer=${farmerId}&page=${page}&page_size=${pageSize}`;
        const amountResponse = await api.get(amountUrl);
        amountData = Array.isArray(amountResponse.data.data)
          ? amountResponse.data.data[0]
          : null;
      ...
      setAdminExpenses(expensesData);
      setTotalCount(expensesData.length);
    } catch (error) {
      let errorMessage = error.message || labels.noExpenses;
      let title = language === "en" ? "Error" : "त्रुटी";
  ...
  const fetchManagerExpenses = async (managerId, page = 1) => {
    setLoading(true);
    try {
      const apiUrl = `/farm/?action=getManagerExpenses&manager=${managerId}&page=${page}&page_size=${pageSize}`;
      const response = await api.get(apiUrl);
  ...
  useEffect(() => {
    if (expenseType === "admin") {
      ...
      fetchManagerExpenses(selectedManagerId, currentPage);
    } else if (expenseType === "manager" && !selectedManagerId) {
      fetchManagers();
      setManagerExpenses([]);
      setManagerTotalAmount(0);
    }
  }, [expenseType, selectedManagerId, currentPage]);
  ...
  const handleManagerSelect = (manager) => {
    ...
    fetchManagerExpenses(manager.id, currentPage);
  };
  ...
  const filteredManagerExpenses = managerExpenses.filter((item) => {
    const searchLower = searchQueryExpenses.toLowerCase();
    return (
      item?.reason?.toLowerCase()?.includes(searchLower) ||
      item?.description?.toLowerCase()?.includes(searchLower) ||
